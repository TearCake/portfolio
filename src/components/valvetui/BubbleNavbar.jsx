import React, { useState, useRef, useEffect, useCallback } from "react";
import "./BubbleNavbar.css";

function BubbleNavbar({
  items = [],
  active = "",
  onSelect,
}) {
  // Find initial active index based on active string prop or default to -1 / 0
  const initialIdx = items.findIndex((item) => item.title === active || item.id === active);
  const [activeIdx, setActiveIdx] = useState(initialIdx !== -1 ? initialIdx : 0);
  const [hoverIdx, setHoverIdx] = useState(null);
  const navRef = useRef(null);
  const linkRefs = useRef([]);

  const [activeBounds, setActiveBounds] = useState({ top: 0, left: 0, width: 0, height: 0 });
  const [hoverBounds, setHoverBounds] = useState({ top: 0, left: 0, width: 0, height: 0 });

  // Synchronize when active prop changes from parent
  useEffect(() => {
    const idx = items.findIndex((item) => item.title === active || item.id === active);
    if (idx !== -1) {
      setActiveIdx(idx);
    }
  }, [active, items]);

  const updateBounds = useCallback(
    (idx, setter) => {
      const el = linkRefs.current[idx];
      const nav = navRef.current;
      if (!el || !nav) return;
      const elRect = el.getBoundingClientRect();
      const navRect = nav.getBoundingClientRect();
      setter({
        top: elRect.top - navRect.top,
        left: elRect.left - navRect.left,
        width: elRect.width,
        height: elRect.height,
      });
    },
    []
  );

  useEffect(() => {
    updateBounds(activeIdx, setActiveBounds);
  }, [activeIdx, updateBounds, items]);

  useEffect(() => {
    if (hoverIdx !== null) {
      updateBounds(hoverIdx, setHoverBounds);
    }
  }, [hoverIdx, updateBounds]);

  const handleClick = (idx, item) => {
    setActiveIdx(idx);
    onSelect?.(item);
  };

  return (
    <div ref={navRef} className="bubble-navbar__wrap">
      {/* Active sliding bubble indicator */}
      <div
        className="bubble-navbar__indicator bubble-navbar__indicator--active"
        style={{
          top: activeBounds.top,
          left: activeBounds.left,
          width: activeBounds.width,
          height: activeBounds.height,
        }}
      />
      {/* Hover sliding bubble indicator */}
      <div
        className="bubble-navbar__indicator bubble-navbar__indicator--hover"
        style={{
          top: hoverBounds.top,
          left: hoverBounds.left,
          width: hoverBounds.width,
          height: hoverBounds.height,
        }}
      />

      <div className="bubble-navbar__nav">
        {items.map((item, i) => (
          <a
            key={item.id || i}
            href={`#${item.id}`}
            ref={(el) => {
              linkRefs.current[i] = el;
            }}
            className={`bubble-navbar__link ${
              i === activeIdx ? "bubble-navbar__link--active" : ""
            }`}
            onClick={() => handleClick(i, item)}
            onMouseEnter={() => setHoverIdx(i)}
            onMouseLeave={() => setHoverIdx(null)}
          >
            {item.title || item.label}
          </a>
        ))}
      </div>
    </div>
  );
}

export { BubbleNavbar };
export default BubbleNavbar;
